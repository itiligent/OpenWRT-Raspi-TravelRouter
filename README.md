...
**FROM AN x86 LINUX SYSTEM**, run the script with `./raspi-travelrouter.sh` and follow the prompts. First you will be prompted for sudo, and then a second prompt will ask to add custom OpenWRT config files. The custom config files are in the included zip file `base-travelrouter-raspi.tar.gz`. Copy the _**unzipped**_ contents of `base-travelrouter-raspi.tar.gz` to the path  `$(pwd)/openwrt_inject_files` that is auto-created by the script, and hit enter to start the build. 
